...
const INPUT_TYPE = { text:'text', email:'email', url:'url', number:'number',
                     date:'date', datetime:'datetime-local', color:'color', password:'password' };

export class FormBuilder {
  /** Build HTML string from schema + optional existing values */
  static build(schema, values = {}) {
    return schema.map(f => FormBuilder._field(f, values)).join('');
  }

  static _field(f, values) {
    if (f.auto && values[f.key] === undefined) return '';   // skip auto on create
    ...
    if (f.type === 'boolean') {
      ...
      ctrl = `<div class="gc-toggle-wrap">
        <label class="gc-toggle">
          <input type="checkbox" name="${f.key}" ${chk} ${readonly ? 'disabled' : ''}>
  ...
  /** Collect form values from a container element */
  ...
  /** Wire up toggle labels inside a container */
