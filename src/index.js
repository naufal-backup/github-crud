...
function makeToaster() {
  ...
  return function (msg, type = 'info', ms = 3500) {
...
export class GithubCRUD {
   ...
   * @param {string}   config.repoOwner   GitHub username/org
   * @param {string}   config.repoName    Repo name for JSON storage
   * @param {string[]} config.roles       Ordered roles, first = most privileged
   * @param {object[]} [config.collections]  Collection definitions (optional)
   * @param {string}   [config.redirectUri]  OAuth callback URL (auto-detected if omitted)
  ...
  /**
   * Call this on every page load. Returns session or null.
  ...
  async getAll(collection)            { return this._db.getAll(this._c(collection)); }
  async query(collection, opts)       { return this._db.query(this._c(collection), opts); }
  async insert(collection, record)    { this._checkPerm('write', collection); return this._db.insert(this._c(collection), record, this._user.login); }
  async update(collection, id, patch) { this._checkPerm('write', collection); return this._db.update(this._c(collection), id, patch, this._user.login); }
  async remove(collection, id)        { this._checkPerm('delete', collection); return this._db.remove(this._c(collection), id, this._user.login); }

  // ── UI — Table ────────────────────────────────────────────────
  /**
   * Mount a full CRUD table into a DOM element.
   * @param {object}           collection  Collection definition
   * @param {string|Element}   container   Where to render the table
   * @param {string|Element}  [modal]      Modal element (auto-created if omitted)
   * @returns {TableRenderer}
   */
  mountTable(collection, container, modal) {
    this._requireDb();
    const col = this._c(collection);

    const modalEl = modal
      ? (typeof modal === 'string' ? document.querySelector(modal) : modal)
      : (() => { const m = Object.assign(document.createElement('div'), { className:'gc-modal' }); document.body.appendChild(m); return m; })();
    ...
    const renderer = new TableRenderer({
      ...
      user:        this._user,
      onStatus:   (s, d) => document.dispatchEvent(new CustomEvent('gc:status', { detail:{ state:s, detail:d, label:col.label } })),
      onToast:    (m, t) => this._toast(m, t),
    });
  ...
  // ── UI — Login card ───────────────────────────────────────────
  /**
   * Render a login card into a container.
  ...
  renderLoginCard(container, opts = {}) {
    ...
    el.innerHTML = `<div class="gc-login-card">
      ...
      <div class="gc-login-desc">${opts.description || `Masuk dengan GitHub untuk melanjutkan.<br/>Hanya collaborator <strong>${repoOwner}/${repoName}</strong> yang dapat akses.`}</div>
  ...
  _checkPerm(action, collection) {
    ...
    if (!ok) throw new Error(`Role "${this._role}" tidak memiliki izin ${action} pada ${col.label}.`);
  }
}
